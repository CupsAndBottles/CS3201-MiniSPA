...
std::vector<pair<int, int>> PKB::getModifies(Enum::TYPE type1, int stmtNum, Enum::TYPE type2, int varIndex)
{
	...
	if (type1 == Enum::PROCEDURE && stmtNum != -1) {
	...
	else if (type1 == Enum::PROCEDURE && stmtNum == -1 && varIndex == -1) {
		// (proc p, x), (
		for (size_t i = 0; i < procTable.size(); i++) {
		...
		for (size_t i = 1; i < stmtTable.size(); i++) {
			if (type1 == stmtTable.at(i).getType() || type1 == 1 ) {
				cout << i << endl;
	...
	if (stmtNum1 != -1) {
			...
			if (stmtNum2 != -1) { // Affects(2, 6)
				...
				if ((stmtTable[stmtNum1].getType() == Enum::TYPE::ASSIGN)) {
					...
					results = De.extractAffectsFirstNum(stmtNum1, modifiesCol, usesCol, nextCol, startEndNum, typeCol, parentTCol, childrenCol);
				} 
			}
		}
...
vector<pair<int, int>> PKB::getAffectsT(Enum::TYPE type1, int stmtNum1, Enum::TYPE type2, int stmtNum2)
{
	...
	if (stmtNum1 != -1) {
		...
		if ((stmtTable[stmtNum2].getType() == Enum::TYPE::ASSIGN)) {
			results = De.extractAffectsTSecondNum(stmtNum2, modifiesCol, usesCol, prevCol, nextCol, startEndNum, typeCol, parentTCol, childrenCol);
		}
	}
	else { // (s/a/_, s/a/_)
